                  ...
                  <tbody>
                     ...
                     <tr>
                        ...
                        @if(Auth::user()->type=='manager' or Auth::user()->type=='assistant' or Auth::user()->type=='admin')<td>
                            <button type="button" class="btn btn-primary" data-toggle="modal" data-target="#exampleModalCenter{{$achat[$i]->id_achat_details}}">Modifier</button>
                            <form method="post" action="/achat/suppression"><input type="hidden" name="_token" value="{{ csrf_token() }}"><button name="btn-supp-achat" id="btn-supp-achat" value="{{$achat[$i]->id_achat_details}}" class="btn btn-danger">Supprimer</button></form>
                        </td>@endif
                        <!-- Modal -->
